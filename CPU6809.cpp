...
Cpu6809::Cpu6809()
{
	...
	// 6809 instruction definitions
	//opCodeLookupTable.resize(256);
	//opCodeLookupTable = { {"NEG", 6, &directMode, &dummyInstruction} };
}
...
/*
uint8_t Cpu6809::flagsToByte()
{
	return cpuReg.CC.E | cpuReg.CC.F | cpuReg.CC.H | cpuReg.CC.I | cpuReg.CC.N | cpuReg.CC.Z | cpuReg.CC.V | cpuReg.CC.C;
}

void Cpu6809::byteToFlags(uint8_t inputByte)
{
	cpuReg.CC.E = (inputByte & 0b10000000);
	cpuReg.CC.F = (inputByte & 0b01000000);
	cpuReg.CC.H = (inputByte & 0b00100000);
	cpuReg.CC.I = (inputByte & 0b00010000);
	cpuReg.CC.N = (inputByte & 0b00001000);
	cpuReg.CC.Z = (inputByte & 0b00000100);
	cpuReg.CC.V = (inputByte & 0b00000010);
	cpuReg.CC.C = (inputByte & 0b00000001);
}
*/
...
void Cpu6809::cpuClockTick()
{
	...
	{
		// Before doing anything else, check if our CPU is in a halt state. If so, return without doing anything else
		if (cpuHaltFlag)
			return;

		// Now check for asserted interrupt signals
			if (assertedInterrupts[INT_RESET])
			{
				cpuReg.PC = gimeBus->readMemoryWord(VECTOR_RESET);
				assertedInterrupts[INT_RESET] = 0;
				cpuCyclesTotal += 2;		// RESET interrupt uses 2 CPU cycles to complete
				cpuHaltFlag = false;
			}
			else if (assertedInterrupts[INT_IRQ] && !cpuReg.CC.I)
			{
				//printf("IRQ occured while at PC of $%04X\n", cpuReg.PC);
				cpuReg.CC.E = true;
		...
		if ((curOpCode == 0x10) || (curOpCode == 0x11))
		{
			...
			extendedOpCodePtrs((curOpCode * 256) + readByteAtCurPC());
		...
		nextOpWaitCounter = curOpCodeCycleCount;	
...
int Cpu6809::addrModeIndexed()
{
	...
	if (paramByte & 0x80)
	{
		...
		case 0b10000000:	// Non-Indirect addressing (Post-increment by 1)
			...
			if ((curOpCode < 0x30) || (curOpCode > 0x33))		// Skip auto-increments for LEAx instructions
...
int Cpu6809::invalidOpCode()
{
	...
	cpuHaltFlag = true;
	printf("Error: Invalid Op code = $%02X. Halted CPU @ $%04X\n", curOpCode, cpuReg.PC);
	return (-1);
}
...
void Cpu6809::RTS(int addrMode)
{
	operandByte = MASK_PC;
	pullFromStack(&cpuReg.S);
	//cpuReg.PC = gimeBus->readMemoryWord(cpuReg.S);
	//cpuReg.S += 2;
}
...
{
	/*
	if ((debuggerRegPC == 0xBA92) || (debugStepMode))
	{
		for (int i = callTraceIndex - 1; i > callTraceIndex - 100; i--)
			printf("Call address = $%04X\n", callTraceArray[i]);
		
		printf("$%04X: %s  %s\n", debuggerRegPC, curOpMnemonic.c_str(), curOpAddressString);
		debugStepMode = true;
		char tempChar = getchar();
		printf("------------------------------\n");
		printf("New Registers and Flags Status\n");
		printf("------------------------------\n");
		printf("A = $%02X, B = $%02X, DP = $%02X, X = $%04X, Y = $%04X, U = $%04X, S = $%04X\n", cpuReg.Acc.A, cpuReg.Acc.B, cpuReg.DP, cpuReg.X, cpuReg.Y, cpuReg.U, cpuReg.S);
		printf("Flags  E F H I N Z V C\n");
		printf("       %u %u %u %u %u %u %u %u\n", cpuReg.CC.E, cpuReg.CC.F, cpuReg.CC.H, cpuReg.CC.I, cpuReg.CC.N, cpuReg.CC.Z, cpuReg.CC.V, cpuReg.CC.C);
		printf("------------------------------\n\n");
		printf("Next instruction @ $%04X\n\n", cpuReg.PC);	
	}

	if (callTraceIndex >= 1024)
		callTraceIndex = 0;
	callTraceArray[callTraceIndex] = debuggerRegPC;
	callTraceIndex++;
	*/
